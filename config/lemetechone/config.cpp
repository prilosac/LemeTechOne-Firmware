...
void setup() {
    // for builds with a brook board connected to gpio1, allow
    // brook board to power on so pins aren't pulled to ground
    // (this was a planned feature for the LemeTechOne and the
    // circuitry is there/prototype is proven, but ultimately the
    // brook board connection + USB-A port was removed/scrapped
    // to make the controller thinner and simplify production
    //gpio_init(1);
    ...
    if (console == ConnectedConsole::NONE) {
        if (button_holds.x) {
            ...
            primary_backend->SetGameMode(new Ultimate(socd::SOCD_2IP, { .mod_z_dpad_left = false } ));
            return;
        }
        else if (button_holds.mod_x) {
            // If no console detected and ModX is held on plugin then use Switch USB backend in FGC mode for Wingman
            NintendoSwitchBackend::RegisterDescriptor();
            backend_count = 1;
            primary_backend = new NintendoSwitchBackend(input_sources, input_source_count);
            backends = new CommunicationBackend *[backend_count] { primary_backend };
